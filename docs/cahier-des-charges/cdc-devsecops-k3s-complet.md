...
### 4.18 Portail unifié (Dashboard DevSecOps)

- Application Web **moderne et futuriste** déployée dans `tools`.
- URL : par exemple `https://devops.entreprise.local`.
- Authentification :
  - SSO via Keycloak.
  - 2FA via Authelia.

#### 4.18.1 Stack technique du portail

- **Frontend** : Framework CSS **Bulma** pour un design épuré, responsive et modulaire.
  - Bulma offre une philosophie minimaliste et moderne, parfaite pour une interface DevOps.
  - Composants : cards, notifications, buttons, tabs, modals.
  - Dark mode/Light mode natif (CSS variables).
  - Responsive design sans dépendance JavaScript lourde.

- **Backend** : **Python** (FastAPI recommandé pour les performances et async/await).
  - FastAPI : framework asynchrone, documentation auto (Swagger/OpenAPI), validation Pydantic.
  - Alternative légère : Flask + Flask-CORS pour plus de simplicité.
  - Serveur WSGI : Gunicorn (production) ou Uvicorn (FastAPI).

- **Image Docker** : base DHI `python:3.12-slim` pour minimiser l'empreinte.
  - Dépendances : FastAPI, requests, python-jose (JWT), redis (cache optionnel).
  - Déploiement K8s : deployment avec liveness/readiness probes.

#### 4.18.2 Fonctionnalités du portail

- **Dashboard personnel** :
  - Affichage personnalisé en fonction du profil utilisateur (dev, PM, ops, manager).
  - Chaque utilisateur ne voit que les outils et données accessibles selon ses rôles Keycloak.

- **Tuiles d'accès (Navigation)** :
  - Icônes + labels vers : GitLab, Harbor, Taiga, Mattermost, Grafana, Portainer, ArgoCD, SonarQube, HedgeDoc, Uptime Kuma.
  - Design en grille Bulma (responsive : 5 colonnes en desktop, 2 en mobile).
  - Indicateurs visuels : badge "nouveau", "alerte" si statut dégradé.

- **Widget "Mes tâches"** :
  - Interrogation API Taiga pour tâches assignées à l'utilisateur.
  - Tri par priorité, échéance, statut (backlog, in progress, done).
  - Clic rapide pour ouvrir dans Taiga.
  - Affichage : liste compacte ou vue kanban légère.

- **Widget "Prochains rendez-vous / échéances"** :
  - Récupération des sprints/échéances Taiga.
  - Affichage calendaire (mini calendrier ou timeline).
  - Alertes visuelles pour les tâches urgentes (rouge si deadline < 1 jour).
  - Intégration avec API Mattermost pour afficher messages épinglés/annonces importantes.

- **Widget "Métriques rapides"** :
  - Résumé des principales métriques Prometheus/Grafana :
    - CPU/RAM cluster (jauge)
    - Erreurs applicatifs (dernières 24h)
    - Déploiements en cours / échoués
  - Clics pour creuser dans Grafana.

- **Widget "État des services"** :
  - Requête API Uptime Kuma pour statut des services clés.
  - Indicateurs visuels (vert OK, orange dégradé, rouge down).
  - Historique raccourci (uptime 7 jours).

- **Widget "Derniers incidents / alertes"** :
  - Feed des dernières alertes Prometheus (dernières 24h).
  - Statut résolu / non-résolu.
  - Lien rapide vers Grafana/Loki/Mattermost.

- **Documentation "Bien démarrer"** :
  - Lien direct vers HedgeDoc.
  - Intégration possible : iframe/embed d'une page HedgeDoc clé.

- **Notifications en temps réel** (optionnel avancé) :
  - WebSocket ou Server-Sent Events (SSE) pour alertes non-bloquantes.
  - Badges de notification (ex. "3 alertes en attente").

#### 4.18.3 Design et UX

- **Thème visuel** :
  - Palette moderne : bleus/teals (devops), gris sombres (dark mode), blanc/crème (light mode).
  - Typographie : Police sans-serif système (Bulma par défaut).
  - Espacement généreux (Bulma spacing utilities).

- **Animations** :
  - Transitions fluides (fade, slide) avec `transition` CSS.
  - Hover effects subtils sur les tuiles (ombre accrue, légère translation).
  - Loading spinners élégants (Bulma loader ou custom SVG).
...
#### 4.18.4 Intégrations API et backend Python

- **Taiga API** :
  ```
  GET /api/v1/userstories?owner={user_id} → tâches
  GET /api/v1/milestones → sprints/échéances
  ```

- **GitLab API** :
  ```
  GET /api/v4/projects/{id}/pipelines → derniers builds
  GET /api/v4/user → profil utilisateur
  ```

- **Uptime Kuma API** :
  ```
  GET /api/status-page/monitors/{id} → état services
  ```

- **Prometheus API** :
  ```
  GET /api/v1/query?query=... → requêtes PromQL pour métriques
  ```

- **Keycloak API** :
  ```
  Token validation (OIDC)
  User info endpoint → groupes, rôles
  ```

- **Backend Python (FastAPI)** :
  - Routes :
    - `GET /dashboard` → données personnalisées (cache Redis 5 min).
    - `GET /tasks` → appel API Taiga + cache.
    - `GET /services-status` → appel API Uptime Kuma.
    - `GET /metrics-summary` → appel Prometheus.
  - Validation tokens Keycloak (python-jose).
  - Gestion CORS (cross-origin pour Traefik).
  - Rate limiting (5 req/sec par utilisateur).

#### 4.18.5 Déploiement et scalabilité

- **Déploiement K8s** :
  - Deployment (replicas: 2 pour HA).
  - Service ClusterIP.
  - Ingress (Traefik) avec host `devops.entreprise.local`.
  - ConfigMap pour variables (URLs APIs, timeouts).
  - Secret pour tokens Keycloak.

- **Performance** :
  - Cache Redis pour tâches/services status (TTL : 1-5 min selon données).
  - Requêtes asynchrones vers APIs externes (non-blocking, timeout 5s).
  - Compression GZIP sur réponses HTTP.

- **Monitoring** :
  - Metrics Prometheus exportées (temps réponse, erreurs API, hits par endpoint).
  - Logs structurés en JSON (stdout → Loki).
...
- **Simplicité opérationnelle** :
  ...
  - Utilisation d'outils facilitant l'exploitation (Portainer, ArgoCD, Grafana, Uptime Kuma, portail unifié).

- **Performances** :
  - Temps de réponse acceptable pour les outils principaux (< 2s pour le portail, < 1s pour GitLab, < 3s pour Taiga).
  - Dimensionnement adapté (CPU/RAM/IO) en fonction de la charge cible (20–30 utilisateurs).

- **Disponibilité** :
  - Objectif de haute disponibilité raisonnable (cluster K3s multi‑nœuds, Longhorn, Velero).
  - SLA cible : 99% (acceptant quelques maintenances planifiées).
...
### 7.1 Phase 1 (MVP – 2 mois)

Déployer le cœur de la plateforme :
- K3s (3 nœuds)
...
- Taiga
- Mattermost
- **Portail unifié** (MVP basique avec Bulma + FastAPI)

### 7.2 Phase 2 (+ 1 mois)

Ajouter sécurité et monitoring :
...
- Uptime Kuma
- Longhorn + Velero
- **Portail unifié** (version complète avec tous les widgets, dark mode, intégrations avancées)

---

Ce cahier des charges décrit de manière détaillée et pragmatique la plateforme DevSecOps cible, en intégrant GitLab, Harbor, K3s, Velero, un portail moderne en Bulma + Python, et l'ensemble des outils de sécurité, de monitoring et de collaboration, avec une forte orientation **DevSecOps** et une expérience utilisateur centralisée et intuitive.